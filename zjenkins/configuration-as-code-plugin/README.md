...
- Path to a folder containing a set of securitydemo.config files. For example, `/var/jenkins_home/casc_configs`.
...
:heavy_check_mark: `/jenkins/casc_configs/dir1/securitydemo.config.yaml`  
:x: `/jenkins/casc_configs/.dir1/securitydemo.config.yaml`  
:x: `/jenkins/casc_configs/..dir2/securitydemo.config.yaml`  
  
`CASC_JENKINS_CONFIG=/jenkins/.configs/casc_configs` contains hidden folder `.securitydemo.config`  
:x: `/jenkins/.configs/casc_configs/jenkins.yaml`  
:x: `/jenkins/.configs/casc_configs/dir1/securitydemo.config.yaml`  
:x: `/jenkins/.configs/casc_configs/.dir1/securitydemo.config.yaml`  
:x: `/jenkins/.configs/casc_configs/..dir2/securitydemo.config.yaml`  
</details>

Instead of setting the `CASC_JENKINS_CONFIG` environment variable, you can also define using
the `casc.jenkins.securitydemo.config` Java property.  This is useful when installing Jenkins via a package
...
`JENKINS_JAVA_OPTIONS` entry in `/etc/sysconfig/jenkins`
  ...
  `-Dcasc.jenkins.securitydemo.config=/jenkins/casc_configs`
 
If you do not set the `CASC_JENKINS_CONFIG` environment variable or the `casc.jenkins.securitydemo.config` Java
property, the plugin will default to looking for a single securitydemo.config file in
