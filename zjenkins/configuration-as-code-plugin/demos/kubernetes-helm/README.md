...
master:
  ...
  sidecars:
    configAutoReload:
      ...
      # over SSH to reapply securitydemo.config when changes to the ConfigScripts are detected.  The admin user (or account you specify in
      ...
      # Is only used to reload JCasC securitydemo.config from the sidecar container running in the Jenkins master pod.
      # This TCP port will not be open in the pod (unless you specifically configure this), so Jenkins will not be
      # accessible via SSH from outside of the pod.  Note if you use non-root pod privileges (RunAsUser & FsGroup),
      # this must be > 1024:
      sshTcpPort: 1044
      # label that the configmaps with Configuration as Code securitydemo.config are marked with:
      label: jenkins_config
      # folder in the pod that should hold the collected Configuration as Code securitydemo.config:
      folder: /var/jenkins_home/casc_configs
      # If specified, the sidecar will search for securitydemo.config-maps inside this namespace.
  ...
  containerEnv:
    # Tell the plugin where to find its securitydemo.config.
