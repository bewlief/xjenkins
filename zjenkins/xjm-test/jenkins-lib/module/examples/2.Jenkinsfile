...
"""
    ...
    stages {
        ...
        stage('Make Branch Image') {
			...
			cp docker_shell/securitydemo.config /home/jenkins/.kube/
            ...
            steps {
                ...
                container('jnlp-slave') {
                    ...
                    withCredentials([usernamePassword(credentialsId: '******-***-***-****-**********', passwordVariable: GIT_PASSWORD, usernameVariable: 'GIT_USERNAME')]) {
                        sh """
			echo ${gittag}
			git securitydemo.config --global user.email "jenkins@xxxxxx.com"
			git securitydemo.config --global user.name "jenkins"
