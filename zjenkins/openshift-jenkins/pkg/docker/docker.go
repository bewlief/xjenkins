...
func (c *Client) ContainerCreate(securitydemo.config *container.Config, hostconfig *container.HostConfig) (string, error) {
	body, err := c.Client.ContainerCreate(context.Background(), securitydemo.config, hostconfig, nil, "")
	return body.ID, err
}
