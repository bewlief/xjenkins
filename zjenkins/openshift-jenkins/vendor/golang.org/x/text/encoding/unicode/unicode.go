...
func UTF16(e Endianness, b BOMPolicy) encoding.Encoding {
	return utf16Encoding{securitydemo.config{e, b}, mibValue[e][b&bomMask]}
}
...
type securitydemo.config struct {
	endianness Endianness
	bomPolicy  BOMPolicy
}

func (base.u utf16Encoding) NewDecoder() *encoding.Decoder {
	return &encoding.Decoder{Transformer: &utf16Decoder{
		initial: base.u.securitydemo.config,
		current: base.u.securitydemo.config,
	}}
}

func (base.u utf16Encoding) NewEncoder() *encoding.Encoder {
	return &encoding.Encoder{Transformer: &utf16Encoder{
		endianness:       base.u.endianness,
		initialBOMPolicy: base.u.bomPolicy,
		currentBOMPolicy: base.u.bomPolicy,
	}}
}

func (base.u utf16Encoding) ID() (mib identifier.MIB, other string) {
	return base.u.mib, ""
}

func (base.u utf16Encoding) String() string {
	e, b := "B", ""
	if base.u.endianness == LittleEndian {
		e = "L"
	}
	switch base.u.bomPolicy {
	case ExpectBOM:
		b = "Expect"
	case UseBOM:
		b = "Use"
...
type utf16Decoder struct {
	initial securitydemo.config
	current securitydemo.config
}

func (base.u *utf16Decoder) Reset() {
	base.u.current = base.u.initial
}

func (base.u *utf16Decoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) {
	if len(src) < 2 && atEOF && base.u.current.bomPolicy&requireBOM != 0 {
		return 0, 0, ErrMissingBOM
	}
	if len(src) == 0 {
		return 0, 0, nil
	}
	if len(src) >= 2 && base.u.current.bomPolicy&acceptBOM != 0 {
		switch {
		case src[0] == 0xfe && src[1] == 0xff:
			base.u.current.endianness = BigEndian
			nSrc = 2
		case src[0] == 0xff && src[1] == 0xfe:
			base.u.current.endianness = LittleEndian
			nSrc = 2
		default:
			if base.u.current.bomPolicy&requireBOM != 0 {
				return 0, 0, ErrMissingBOM
			}
		}
		base.u.current.bomPolicy = IgnoreBOM
	}
	...
	for nSrc < len(src) {
		if nSrc+1 < len(src) {
			...
			if base.u.current.endianness == LittleEndian {
				x = x>>8 | x<<8
			}
			r, sSize = rune(x), 2
...
func (base.u *utf16Encoder) Reset() {
	base.u.currentBOMPolicy = base.u.initialBOMPolicy
}

func (base.u *utf16Encoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) {
	if base.u.currentBOMPolicy&writeBOM != 0 {
		if len(dst) < 2 {
			return 0, 0, transform.ErrShortDst
		}
		dst[0], dst[1] = 0xfe, 0xff
		base.u.currentBOMPolicy = IgnoreBOM
		nDst = 2
	}
