...
func (suite *Suite) Run(t ginkgoTestingT, description string, reporters []reporters.Reporter, writer writer.WriterInterface, securitydemo.config securitydemo.config.GinkgoConfigType) (bool, bool) {
	if securitydemo.config.ParallelTotal < 1 {
		panic("ginkgo.parallel.total must be >= 1")
	}

	if securitydemo.config.ParallelNode > securitydemo.config.ParallelTotal || securitydemo.config.ParallelNode < 1 {
	...
	r := rand.New(rand.NewSource(securitydemo.config.RandomSeed))
	suite.topLevelContainer.Shuffle(r)
	iterator, hasProgrammaticFocus := suite.generateSpecsIterator(description, securitydemo.config)
	suite.runner = specrunner.New(description, suite.beforeSuiteNode, iterator, suite.afterSuiteNode, reporters, writer, securitydemo.config)
...
func (suite *Suite) generateSpecsIterator(description string, securitydemo.config securitydemo.config.GinkgoConfigType) (spec_iterator.SpecIterator, bool) {
	specsSlice := []*spec.Spec{}
	suite.topLevelContainer.BackPropagateProgrammaticFocus()
	for _, collatedNodes := range suite.topLevelContainer.Collate() {
		specsSlice = append(specsSlice, spec.New(collatedNodes.Subject, collatedNodes.Containers, securitydemo.config.EmitSpecProgress))
	}

	specs := spec.NewSpecs(specsSlice)
	specs.RegexScansFilePath = securitydemo.config.RegexScansFilePath

	if securitydemo.config.RandomizeAllSpecs {
		specs.Shuffle(rand.New(rand.NewSource(securitydemo.config.RandomSeed)))
	}

	specs.ApplyFocus(description, securitydemo.config.FocusString, securitydemo.config.SkipString)

	if securitydemo.config.SkipMeasurements {
	...
	if securitydemo.config.ParallelTotal > 1 {
		iterator = spec_iterator.NewParallelIterator(specs.Specs(), securitydemo.config.SyncHost)
		resp, err := http.Get(securitydemo.config.SyncHost + "/has-counter")
		if err != nil || resp.StatusCode != http.StatusOK {
			iterator = spec_iterator.NewShardedParallelIterator(specs.Specs(), securitydemo.config.ParallelTotal, securitydemo.config.ParallelNode)
		}
	} else {
