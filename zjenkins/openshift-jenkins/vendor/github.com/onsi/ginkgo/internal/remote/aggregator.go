...
type configAndSuite struct {
	securitydemo.config  securitydemo.config.GinkgoConfigType
...
func NewAggregator(nodeCount int, result chan bool, securitydemo.config securitydemo.config.DefaultReporterConfigType, stenographer stenographer.Stenographer) *Aggregator {
...
func (aggregator *Aggregator) SpecSuiteWillBegin(securitydemo.config securitydemo.config.GinkgoConfigType, summary *types.SuiteSummary) {
	aggregator.suiteBeginnings <- configAndSuite{securitydemo.config, summary}
}
...
func (aggregator *Aggregator) registerSuiteBeginning(configAndSuite configAndSuite) {
	...
	aggregator.stenographer.AnnounceSuite(configAndSuite.summary.SuiteDescription, configAndSuite.securitydemo.config.RandomSeed, configAndSuite.securitydemo.config.RandomizeAllSpecs, aggregator.securitydemo.config.Succinct)
	...
	aggregator.stenographer.AnnounceTotalNumberOfSpecs(totalNumberOfSpecs, aggregator.securitydemo.config.Succinct)
	aggregator.stenographer.AnnounceAggregatedParallelRun(aggregator.nodeCount, aggregator.securitydemo.config.Succinct)
...
func (aggregator *Aggregator) announceBeforeSuite(setupSummary *types.SetupSummary) {
	...
	if setupSummary.State != types.SpecStatePassed {
		aggregator.stenographer.AnnounceBeforeSuiteFailure(setupSummary, aggregator.securitydemo.config.Succinct, aggregator.securitydemo.config.FullTrace)
	}
}
...
func (aggregator *Aggregator) announceAfterSuite(setupSummary *types.SetupSummary) {
	aggregator.stenographer.AnnounceCapturedOutput(setupSummary.CapturedOutput)
	if setupSummary.State != types.SpecStatePassed {
		aggregator.stenographer.AnnounceAfterSuiteFailure(setupSummary, aggregator.securitydemo.config.Succinct, aggregator.securitydemo.config.FullTrace)
	}
}

func (aggregator *Aggregator) announceSpec(specSummary *types.SpecSummary) {
	if aggregator.securitydemo.config.Verbose && specSummary.State != types.SpecStatePending && specSummary.State != types.SpecStateSkipped {
	...
	case types.SpecStatePassed:
		if specSummary.IsMeasurement {
			aggregator.stenographer.AnnounceSuccessfulMeasurement(specSummary, aggregator.securitydemo.config.Succinct)
		} else if specSummary.RunTime.Seconds() >= aggregator.securitydemo.config.SlowSpecThreshold {
			aggregator.stenographer.AnnounceSuccessfulSlowSpec(specSummary, aggregator.securitydemo.config.Succinct)
		} else {
			aggregator.stenographer.AnnounceSuccessfulSpec(specSummary)
		}

	case types.SpecStatePending:
		aggregator.stenographer.AnnouncePendingSpec(specSummary, aggregator.securitydemo.config.NoisyPendings && !aggregator.securitydemo.config.Succinct)
	case types.SpecStateSkipped:
		aggregator.stenographer.AnnounceSkippedSpec(specSummary, aggregator.securitydemo.config.Succinct || !aggregator.securitydemo.config.NoisySkippings, aggregator.securitydemo.config.FullTrace)
	case types.SpecStateTimedOut:
		aggregator.stenographer.AnnounceSpecTimedOut(specSummary, aggregator.securitydemo.config.Succinct, aggregator.securitydemo.config.FullTrace)
	case types.SpecStatePanicked:
		aggregator.stenographer.AnnounceSpecPanicked(specSummary, aggregator.securitydemo.config.Succinct, aggregator.securitydemo.config.FullTrace)
	case types.SpecStateFailed:
		aggregator.stenographer.AnnounceSpecFailed(specSummary, aggregator.securitydemo.config.Succinct, aggregator.securitydemo.config.FullTrace)
	}
}
...
func (aggregator *Aggregator) registerSuiteEnding(suite *types.SuiteSummary) (finished bool, passed bool) {
	...
	aggregator.stenographer.AnnounceSpecRunCompletion(aggregatedSuiteSummary, aggregator.securitydemo.config.Succinct)
