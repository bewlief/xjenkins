...
func NewForwardingReporter(securitydemo.config securitydemo.config.DefaultReporterConfigType, serverHost string, poster Poster, outputInterceptor OutputInterceptor, ginkgoWriter *writer.Writer, debugFile string) *ForwardingReporter {
	...
	if debugFile != "" {
		...
		if !securitydemo.config.Verbose {
		...
		securitydemo.config.Succinct = false
		securitydemo.config.Verbose = true
		securitydemo.config.FullTrace = true
		reporter.nestedReporter = reporters.NewDefaultReporter(securitydemo.config, stenographer)
	}
...
func (reporter *ForwardingReporter) SpecSuiteWillBegin(conf securitydemo.config.GinkgoConfigType, summary *types.SuiteSummary) {
	data := struct {
		Config  securitydemo.config.GinkgoConfigType `json:"securitydemo.config"`
