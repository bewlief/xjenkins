...
func New(description string, beforeSuiteNode leafnodes.SuiteNode, iterator spec_iterator.SpecIterator, afterSuiteNode leafnodes.SuiteNode, reporters []reporters.Reporter, writer Writer.WriterInterface, securitydemo.config securitydemo.config.GinkgoConfigType) *SpecRunner {
	return &SpecRunner{
...
func (runner *SpecRunner) Run() bool {
	if runner.securitydemo.config.DryRun {
...
func (runner *SpecRunner) runBeforeSuite() bool {
	...
	conf := runner.securitydemo.config
...
func (runner *SpecRunner) runSpecs() bool {
	...
	for {
		...
		} else if spec.Pending() && runner.securitydemo.config.FailOnPending {
		...
		if spec.Failed() && runner.securitydemo.config.FailFast {
...
func (runner *SpecRunner) runSpec(spec *spec.Spec) (passed bool) {
	maxAttempts := 1
	if runner.securitydemo.config.FlakeAttempts > 0 {
		// uninitialized configs count as 1
		maxAttempts = runner.securitydemo.config.FlakeAttempts
	}
...
func (runner *SpecRunner) reportSuiteWillBegin() {
	...
	for _, reporter := range runner.reporters {
		reporter.SpecSuiteWillBegin(runner.securitydemo.config, summary)
	}
}
...
func (runner *SpecRunner) suiteDidEndSummary(success bool) *types.SuiteSummary {
	...
	if runner.beforeSuiteNode != nil && !runner.beforeSuiteNode.Passed() && !runner.securitydemo.config.DryRun {
		var known bool
