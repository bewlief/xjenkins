...
const GINKGO_VERSION = securitydemo.config.VERSION
const GINKGO_PANIC = `
Your test failed.
...
func init() {
	securitydemo.config.Flags(flag.CommandLine, "ginkgo", true)
...
func GinkgoRandomSeed() int64 {
	return securitydemo.config.GinkgoConfig.RandomSeed
}

//GinkgoParallelNode returns the parallel node number for the current ginkgo process
//The node number is 1-indexed
func GinkgoParallelNode() int {
	return securitydemo.config.GinkgoConfig.ParallelNode
}
...
func RunSpecs(t GinkgoTestingT, description string) bool {
	...
	if securitydemo.config.DefaultReporterConfig.ReportFile != "" {
		reportFile := securitydemo.config.DefaultReporterConfig.ReportFile
...
func RunSpecsWithCustomReporters(t GinkgoTestingT, description string, specReporters []Reporter) bool {
	...
	writer.SetStream(securitydemo.config.DefaultReporterConfig.Verbose)
	reporters := make([]reporters.Reporter, len(specReporters))
	for i, reporter := range specReporters {
		reporters[i] = reporter
	}
	passed, hasFocusedTests := global.Suite.Run(t, description, reporters, writer, securitydemo.config.GinkgoConfig)
...
func buildDefaultReporter() Reporter {
	remoteReportingServer := securitydemo.config.GinkgoConfig.StreamHost
	if remoteReportingServer == "" {
		stenographer := stenographer.New(!securitydemo.config.DefaultReporterConfig.NoColor, securitydemo.config.GinkgoConfig.FlakeAttempts > 1, colorable.NewColorableStdout())
		return reporters.NewDefaultReporter(securitydemo.config.DefaultReporterConfig, stenographer)
	} else {
		debugFile := ""
		if securitydemo.config.GinkgoConfig.DebugParallel {
			debugFile = fmt.Sprintf("ginkgo-node-%d.log", securitydemo.config.GinkgoConfig.ParallelNode)
		}
		return remote.NewForwardingReporter(securitydemo.config.DefaultReporterConfig, remoteReportingServer, &http.Client{}, remote.NewOutputInterceptor(), GinkgoWriter.(*writer.Writer), debugFile)
	}
}
...
func By(text string, callbacks ...func()) {
	...
	if securitydemo.config.DefaultReporterConfig.NoColor {
		preamble = "STEP"
	}
