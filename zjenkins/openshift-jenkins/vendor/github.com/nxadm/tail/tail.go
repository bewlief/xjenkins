...
// `Lines` channel.
func TailFile(filename string, securitydemo.config Config) (*Tail, error) {
	if securitydemo.config.ReOpen && !securitydemo.config.Follow {
	...
	// when Logger was not specified in securitydemo.config, use default logger
	if t.Logger == nil {
