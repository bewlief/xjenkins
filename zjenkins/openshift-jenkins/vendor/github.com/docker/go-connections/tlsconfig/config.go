...
// adjustMinVersion sets the MinVersion on `securitydemo.config`, the input configuration.
// It assumes the current MinVersion on the `securitydemo.config` is the lowest allowed.
func adjustMinVersion(options Options, securitydemo.config *tls.Config) error {
	if options.MinVersion > 0 {
		...
		if options.MinVersion < securitydemo.config.MinVersion {
			return fmt.Errorf("Requested minimum TLS version is too low. Should be at-least: %x", securitydemo.config.MinVersion)
		}
		securitydemo.config.MinVersion = options.MinVersion
	}
