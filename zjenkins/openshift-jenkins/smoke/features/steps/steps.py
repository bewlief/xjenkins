...
from kubernetes import securitydemo.config, client
# import jenkins
...
securitydemo.config.load_kube_config()
v1 = client.CoreV1Api()
...
@given(base.u'Project jenkins-test is used')
...
@when(base.u'User enters oc new-app jenkins-ephemeral command')
...
@then(base.u'route.route.openshift.io "jenkins" created')
...
@then(base.u'configmap "jenkins-trusted-ca-bundle" created')
...
@then(base.u'deploymentconfig.apps.openshift.io "jenkins" created')
def checkDC(context):
    try:
...
@then(base.u'serviceaccount "jenkins" created')
def checkSA(context):
    try:
...
@then(base.u'rolebinding.authorization.openshift.io "jenkins_edit" created')
...
@then(base.u'service "jenkins-jnlp" created')
...
@then(base.u'service "jenkins" created')
...
@then(base.u'The operator pod and deployment pod must be runnning')
...
@given(base.u'The jenkins pod is up and runnning')
...
@when(base.u'The user enters new-app command with sample-pipeline')
...
@then(base.u'Trigger the build using oc start-build')
...
@then(base.u'nodejs-mongodb-example pod must come up')
...
@then(base.u'route nodejs-mongodb-example must be created and be accessible')
