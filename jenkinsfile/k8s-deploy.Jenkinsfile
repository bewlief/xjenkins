...
pipeline {
    ...
    environment {
        ...
        K8S_CONFIG = credentials('jenkins-k8s-securitydemo.config')
    ...
    stages {
        ...
        stage('Docker Build') {
            ...
            steps {
                unstash 'app'
                sh "docker login -base.u ${HARBOR_CREDS_USR} -p ${HARBOR_CREDS_PSW} ${params.HARBOR_HOST}"
        ...
        stage('Deploy') {
            ...
            steps {
                sh "mkdir -p ~/.kube"
                sh "echo ${K8S_CONFIG} | base64 -d > ~/.kube/securitydemo.config"
